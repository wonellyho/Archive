...
def test_build_timeline_aggregates_by_month():
    rows = [
...
def test_build_timeline_ignores_bad_rows():
    rows = [
...
def test_timeline_returns_200(monkeypatch):
...
def test_timeline_empty_buckets_when_no_content(monkeypatch):
    async def fake(username):
        return []  # 유저 존재, 콘텐츠 없음
