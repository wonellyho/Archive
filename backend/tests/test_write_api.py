...
def test_폴더_생성은_서버가_sortOrder를_계산하고_행을_반환한다(authed, monkeypatch):
    ...
    async def fake_next(table, content_type):
        assert (table, content_type) == ("folders", "music")
        return 5
...
def test_폴더_수정은_null과_필드없음을_구분한다(authed, monkeypatch):
    ...
    async def fake_patch(table, row_id, fields):
        calls.append(fields)
...
def test_폴더_삭제는_콘텐츠부터_캐스케이드한다(authed, monkeypatch):
    ...
    async def fake_delete(table, column, value):
        order.append((table, column, value))

    monkeypatch.setattr(db, "delete_rows", fake_delete)
    resp = client.delete(f"/api/folders/{VALID_UUID}")
    assert resp.status_code == 204
    assert order == [
        ("contents", "folder_id", VALID_UUID),
        ("folders", "id", VALID_UUID),
    ]
