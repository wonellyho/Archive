...
def test_missing_key_returns_503(authed, monkeypatch):
    class _S:
        anthropic_api_key = ""
...
def test_success_returns_camel_case(authed, monkeypatch):
    fake = FakeProvider(
...
def test_provider_error_returns_502(authed, monkeypatch):
...
def test_provider_rate_limit_returns_429(authed, monkeypatch):
...
def test_user_rate_limit_skips_provider(authed, monkeypatch):
...
def test_parse_result_enforces_count_and_length():
    long = "글" * 60
    r = parse_result(
...
def test_parse_result_invalid_output_raises(text):
...
def test_parse_result_parses_keywords_and_tone():
    r = parse_result(
        '{"taglines": ["가"], "mood": "잔잔", '
...
def test_parse_result_defaults_keywords_and_tone():
