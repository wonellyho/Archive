...
# 유해어 세트 — 포함 시 해당 문구 제거.
# ⚠️ 부분 문자열 매칭이므로, benign 한국어와 충돌하는 표현은 넣지 않는다
#    (예: '새끼'→"고양이 새끼", '꺼져'→"촛불이 꺼져", '시발'→"시발점"). 이 필터는
#    백스톱이고 1차 안전성은 모델(Claude) 자체가 담당하므로, 명백한 욕설/슬러만 둔다.
_HARMFUL = (
    "씨발", "씨빨", "개새끼", "병신", "지랄", "좆같", "니미",
    "fuck", "bitch", "asshole", "motherfuck",
)
